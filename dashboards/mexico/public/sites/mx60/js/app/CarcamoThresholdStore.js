/**
 * MX60.CarcamoThresholdStore
 *
 * Read-through cache + write-through for cárcamo level thresholds.
 * Station Niagara slots are the single source of truth (REQ-G4-CARCAMO).
 * Comparison is INVERTED — for cárcamos, lectura > umbral is degraded state.
 *
 * Keys per equipId: umbralAdvertencia, umbralCritico (numbers in %).
 * Defaults: 45% advertencia, 69% crítico.
 *
...
(function () {
  ...
  var DEFAULTS = { umbralAdvertencia: 45, umbralCritico: 69 };
  ...
  function getEffective(equipId, thresholdKey) {
    var v = get(equipId, thresholdKey);
    if (v !== null && v !== undefined) return v;
    return DEFAULTS[thresholdKey];
  }

  /**
   * C3: init — fetch thresholds from station via GET /api/carcamo/{ord}/thresholds.
   * REQ-G4-CARCAMO AC-5: store MUST NOT initialize from localStorage.
   */
  function init(equipId, ord, onDone) {
    ...
    xhr.open('GET', '/mx60/api/carcamo/' + encodeURIComponent(ord) + '/thresholds', true);
   ...
   * Write-through: station is source of truth (REQ-G4-CARCAMO AC-4).
   */
  function set(equipId, thresholdKey, value, ord) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open('POST', '/mx60/api/carcamo/' + encodeURIComponent(ord) + '/threshold', true);
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
    ...
    xhr.onreadystatechange = function () {
      ...
      if (xhr.status === 200) {
        ...
        console.warn('[CarcamoThresholdStore] set failed HTTP ' + xhr.status + ' for ' + thresholdKey);
      }
    };
    xhr.send(body);
  }
  ...
  /**
   * Resolve color class for a cárcamo reading using effective thresholds
   * (override, falling back to DEFAULTS). Comparison is INVERTED relative to
   * datalogger: `lectura > umbral` triggers the degraded color.
   */
  function colorForReading(equipId, lectura) {
    if (lectura === null || lectura === undefined || isNaN(lectura)) return 'verde';
    var crit = getEffective(equipId, 'umbralCritico');
    var adv  = getEffective(equipId, 'umbralAdvertencia');
  ...
  /**
   * C2: seedFromEquipment — hydrate from the equipment REST API response.
   *
   * For each equipment item of type 'carcamo', extracts umbralAdvertencia and
   * umbralCritico from item.summary. Null values (0.0 or unset) are skipped.
   * Emits 'seeded' event after processing (REQ-005, REQ-701).
   */
  function seedFromEquipment(equipArr) {
    ...
    for (var i = 0; i < equipArr.length; i++) {
      var eq = equipArr[i];
      if (!eq || eq.type !== 'carcamo' || !eq.id) continue;
      var s = eq.summary || {};
      for (var k = 0; k < VALID_KEYS.length; k++) {
        var key = VALID_KEYS[k];
        var val = s[key];
    ...
    if (count > 0) console.log('[CarcamoThresholdStore] seeded ' + count + ' items from server');
    _emit({ type: 'seeded', count: count });
  }

  window.MX60.CarcamoThresholdStore = {
    DEFAULTS: DEFAULTS,
    get: get,
    getAll: getAll,
