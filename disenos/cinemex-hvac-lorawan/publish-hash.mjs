 ...
 * hashed files themselves cache immutably (see publish/_headers).
 *
 * SHARED-BY-COPY: the DHL sibling builder keeps its own byte-identical copy of this file next to
 * its build-publish.mjs, exactly as the two builders already mirror each other. Keep them in sync
 * by hand — neither imports across project roots.
 *
 ...
 * A `?query` (e.g. a stale `?v=20260719a` from the old manual patch) is DROPPED for mapped refs —
 * the content hash is the only cache key we want.
 */
 ...
 * The dynamic `import('./main.js')` bootstrap in the cinemex index and any leftover `?v=` query
 * strings are both covered.
 */
