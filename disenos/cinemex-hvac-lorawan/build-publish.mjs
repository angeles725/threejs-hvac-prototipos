...
// Per-project shared-key access gate (see gate.mjs). Loaded once; injected into every shipped page
// so a direct link opens the passcode overlay before the viewer boots. gate-keys.json lives here
// (this dir owns it); keygen.mjs generates/rotates it. Fails loud if the keys were never generated.
const GATE = loadGateConfig(join(ROOT, 'gate-keys.json'), 'cinemex');
