 ...
 *   - content-hashed bundles/styles (one splat = the hash) -> max-age=600, NOT immutable.
 *     A hashed filename argues for immutable, and that is what this used to be. It was wrong:
 *     Pages answers 200 with the PORTAL HTML for an asset it cannot find yet, and in the
 *     seconds a deploy takes to propagate, a request for a brand-new bundle gets that HTML
 *     frozen under the bundle's URL for a year. Measured 2026-08-04 on
 *     /p/hotspot/rack-detail.72dfaa09.js: 200, content-type text/html, cf-cache-status HIT,
 *     immutable — the 3D scene never loaded because its module was the portal. A short
 *     max-age keeps the hash's benefit while letting a poisoned entry heal by itself.
 *   - unhashed assets (logos, favicons) -> short revalidate.
 *   - the per-project protection.js -> content-hashed, same short max-age as the bundles: the
...
/p/hotspot/rack-detail.*.js
  Cache-Control: public, max-age=600
/p/hotspot-sincdn/main.*.js
  Cache-Control: public, max-age=600
/p/hotspot-sincdn/rack-detail.*.js
  Cache-Control: public, max-age=600
/p/hotspot-sincdn/protection.*.js
  Cache-Control: public, max-age=600
/p/hotspot-sincdn/vendor/*
  Cache-Control: public, max-age=600
/p/kalte/gobernador/main.*.js
  Cache-Control: public, max-age=600
/p/kalte/gobernador/sim.*.js
  Cache-Control: public, max-age=600
...
/p/hotspot/protection.*.js
  Cache-Control: public, max-age=600
/p/kalte/gobernador/protection.*.js
  Cache-Control: public, max-age=600
...
console.log(`  publish/_headers written; hashed assets cache for 10 min (not immutable — see the policy note), HTML stays no-cache`);
