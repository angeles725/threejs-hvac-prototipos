...
| | |
|---|---|
| Viewer | **[`nave-panccadia-3d-v15.html`](nave-panccadia-3d-v15.html)** — v14 plus a **wall-corner join**: `wall_rect` stopped each box at the centreline endpoint, so wherever the drawing's centrelines did not meet exactly (46 % of ground-floor ends) an L- or T-corner opened into a notch and the fabric fell into 60 pieces. Each end is now extended half a thickness **only where another run is within 0.20 m** (a corner), so the boxes lap and the union fills the square; genuine free ends (doorways) are left alone and re-cut. Ground-floor fabric drops from **60 → 36 pieces**, 1.81 m² of overlap dissolved, and the audit still reads 0 swallowed / 0 coplanar jambs. v14 kept the HUD-honesty fix: the "Certified levels → Roof" row reads `+9.20 m nominal · gable apex +8.54…+9.48 m` for the pitched roof. The v13 build under it: columns **trimmed to the roof surface**, doorways **cut back out of the wall-solid union**, nested double-outline equipment/furniture undrawn, facade **fitted to its face**. A cross-class geometric auditor (`geo-audit.py`) gates the build: **0 visible defects, 7 technical residues** — the three "nested equipment" residues are real occluded geometry (the laminadora's `MESA` and each rotary oven's side detail, `max_bbox_poke = 0.0 mm`), confirmed against the DXF, so collapsing them would erase real objects. Supersedes `v14`, `v13`, `v12`, `v10`, `v9`.<br>[`nave-panccadia-3d-v14.html`](nave-panccadia-3d-v14.html) · [`nave-panccadia-3d-v13.html`](nave-panccadia-3d-v13.html) · [`nave-panccadia-3d-v12.html`](nave-panccadia-3d-v12.html) · [`nave-panccadia-3d-v10.html`](nave-panccadia-3d-v10.html) · [`nave-panccadia-3d-v9.html`](nave-panccadia-3d-v9.html) — self-contained, opens from the filesystem by double-click. |
