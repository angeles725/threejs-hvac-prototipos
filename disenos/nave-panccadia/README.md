...
| | |
|---|---|
| Viewer | **[`nave-panccadia-3d-v16.html`](nave-panccadia-3d-v16.html)** — v15 plus **wall regularisation + one merged mesh**, so the fabric reads as one construction instead of loose panels. The traced centrelines wobble a few cm, so runs that are ONE wall were drawn as near-parallel lines a few cm apart; their boxes then unioned into stepped, doubled edges (the notched corners in close-up). `regularize_walls` groups near-collinear runs, holds the LONGEST run of each group fixed (it carries the doorways — it must not move), and projects the shorter offset runs laterally onto its line — length and along-line position preserved, so no run is shortened or dropped. Stepped pairs fall from **32 → 10**; the audit still reads **0 visible / 7 technical, 0 swallowed / 0 coplanar jambs** (the anchor rule keeps every doorway host in place). Each storey's walls are then emitted as ONE `BufferGeometry`, so there is no inter-mesh shadow seam at junctions. v14/v15 kept: honest roof HUD (`+9.20 m nominal · gable apex +8.54…+9.48 m`) and the corner-join. The v13 build under it: columns **trimmed to the roof surface**, doorways **cut back out of the union**, facade **fitted to its face**. Auditor `geo-audit.py`: **0 visible defects, 7 technical residues** (the three "nested equipment" are real occluded geometry — laminadora `MESA`, oven side detail — confirmed against the DXF). Supersedes `v15`, `v14`, `v13`, `v12`, `v10`, `v9`.<br>[`nave-panccadia-3d-v15.html`](nave-panccadia-3d-v15.html) · [`nave-panccadia-3d-v14.html`](nave-panccadia-3d-v14.html) · [`nave-panccadia-3d-v13.html`](nave-panccadia-3d-v13.html) · [`nave-panccadia-3d-v9.html`](nave-panccadia-3d-v9.html) — self-contained, opens from the filesystem by double-click. |
