...
> **Amended in v21.** That still describes the reconstruction itself, but it is no longer true of
> everything in this folder: the cold-room amendment WAS authored from a DesignSpec and gated, under
> [`camaras-frigorificas/`](camaras-frigorificas/) (spec, `shots.json`, eight captures and a P6
> verdict in `runs/progress.yaml`). Reconstruction and authored amendments now coexist here.

| | |
|---|---|
| Viewer | **[`nave-panccadia-3d-v21.html`](nave-panccadia-3d-v21.html)** — **the cold rooms become cold rooms.** The three refrigerated rooms are lined with sandwich panel derived from their own flood-filled RUNS outline (vertical joints at the commercial 1.00 m useful width, ceiling at the measured 2.447 m head and hidden by default so the lining stays legible), the four doorways that serve them carry **insulated doors** instead of the plant's conventional brown leaf (lacquered frame, EPDM perimeter bead, barrel hinges, lever latch, 304 stainless leaf, and on the freezer a PVC strip curtain and heated frame trim), and **nine of the drawn machines stop being flat green boxes** — ultracongelador, laminadora + bench, three amasadoras, two batidoras — ported from the gated masters in `equipos/`. Swing sides, facing and wall-hugging are solved from GEOMETRY, not typed: 0 clashing leaf pairs, 5 of 8 machines turned to face their free floor. Also adds a **"Pick coordinates"** HUD tool that raycasts a click into CAD (x, y) + height for placing further equipment. Verified headless: 8 gate captures, `console_clean` 8 of 8. **Its P6 blind review FAILED on structural (0.72 against a 0.78 threshold)** — see [`camaras-frigorificas/runs/progress.yaml`](camaras-frigorificas/runs/progress.yaml): a 0.257 m gap is left open above every cold-door leaf, which every console, framing and count check passes over. Materials passes at 0.78; a second finding written up against it was **retracted on measurement** (the stainless was compared to its albedo hex instead of the kit's documented rendered target, and its supporting sample turned out to be a painted dielectric, not the steel) and is kept in the file as a corrected row. Evidence: corpus [Block 44]; spec: [`camaras-frigorificas/design-spec.yaml`](camaras-frigorificas/design-spec.yaml). Supersedes `v20`.<br>**[`nave-panccadia-3d-v20.html`](nave-panccadia-3d-v20.html)** — **one drawn source of truth for the walls.** Same drawn geometry and audit as v19 (caps 47.696040 == 47.696040 m², re-anchor 0/20, leaf clip 0), but the doorway re-anchoring, the leaf-clip and the HUD now **measure the same regularised+extended wall centrelines the fabric is drawn from**, instead of a raw un-extended collapse. This closes the v18/v19 latent double-pipeline: one more doorway frame is now honestly detected as needing to move onto its wall (re-anchored 4 → 5 of 20, max offset 74 mm, 0 openings lost their host), and the HUD "Wall runs" length reports the drawn walls (315 m) rather than the raw ones (308 m). Verified headless: WebGL2, `glError 0`, zero console messages. Supersedes `v19`, `v18`.<br>**[`nave-panccadia-3d-v19.html`](nave-panccadia-3d-v19.html)** — **v18 geometry, one render-hygiene fix.** Byte-for-byte the same reconstruction as v18 (same checks: caps triangulate 47.696040 == 47.696040 m², 0 invalid rings, re-anchor 0/20, leaf clip 0 penetrations); the only change is a **one-line memory-leak fix** — the `PMREMGenerator` used to bake the image-based lighting is now `dispose()`d after the env map is generated, releasing its render target (it leaked once at load in v17/v18). Verified in a real browser: WebGL2, `glError 0`, zero console messages, IBL intact. Supersedes `v18`.<br>**[`nave-panccadia-3d-v18.html`](nave-panccadia-3d-v18.html)** — a **realistic render + clean corners**, benchmarked against a Revit/IFC model of a comparable plant (`tools/ifc-to-viewer.py` reads that IFC so the two can be judged side by side; Revit's clean look is precise geometry + lighting, not photorealism). Two changes: **(1) rendering** — image-based lighting from a procedural `RoomEnvironment`, `ACESFilmicToneMapping`, and sRGB output turn the flat schematic into an architectural render (no external HDRI, still self-contained); **(2) corners** — `extend_to_corners` pushes a free wall end **along its own axis** to the near-perpendicular run it turns into, so L-corners the drawing left open by up to 45 cm close into a shared corner without moving any centreline sideways or shortening a run. Open L-corners fall **38 → 7** (on top of v16's collinear-step fix, 32 → 10). The audit holds at **0 visible defects** (8 technical, all benign; 0 swallowed / 0 coplanar jambs — doorway hosts and jambs untouched). Under it: regularised + single-merged-mesh walls (v16), honest roof HUD (`+9.20 m nominal · gable apex +8.54…+9.48 m`), columns trimmed to the roof, doorways cut back out of the union. Supersedes `v16`, `v15`, `v14`, `v13`, `v9`.<br>[`nave-panccadia-3d-v17.html`](nave-panccadia-3d-v17.html) (render only) · [`nave-panccadia-3d-v16.html`](nave-panccadia-3d-v16.html) · [`nave-panccadia-3d-v15.html`](nave-panccadia-3d-v15.html) · [`nave-panccadia-3d-v13.html`](nave-panccadia-3d-v13.html) — self-contained, opens from the filesystem by double-click. |
...
| Research corpus | `~/investigacion/nave-panccadia` (Research-SDD target #18, **44 cited blocks**, read-only investigable coverage 39/39 — corpus EXHAUSTED; B38/B39 certify the v9 build, B43 the v14–v18 refinements, B44 the v21 cold rooms) |
...
| **Openings** | the 20 openings with their lintels — the doorways the drawing marks by STOPPING the wall |
...
| ~~Only 9 doors~~ **FIXED in v6** | The operator counted 21; the model had 8. Six searches for the door SYMBOL failed because **the drawing marks most doors by STOPPING THE WALL**, not with a symbol. Reconstructing each wall line and reading the gaps between its stretches gives **20 openings**, widths 0.75–2.98 m. Calibrated, not tuned: the cold rooms have five doors by independent count and the detector finds five, at every setting of a 2.5× tolerance sweep. Two injected defects break that calibration (6/5 and 4/5), so the check is a guard that has been seen to fail. |
