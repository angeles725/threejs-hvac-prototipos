...
//
// A DOM button is a control panel, not an interaction with the OBJECT. This pass makes the
// UPS itself clickable — and on this subject that is unusually literal: the two hotspots
// on the front are the two things a person actually touches on a real UPS, its power
// button and its touch screen, and both drive the same power state the hardware would.
//
// PORTED WITH ITS FIX. The occlusion test here already contains the correction the sibling
// asset needed: pick() stops at the FIRST visible surface rather than skipping non-hotspot
// hits and walking through solid geometry. That bug only surfaced when the occluder was
// ordinary geometry, which is the common case — and it is the case here, where the chassis
// body sits between the camera and the rear panel.
//
...
function candidatePoints(root, limit = 64) {
  ...
  root.traverse((o) => {
    ...
    if (o.isInstancedMesh) {
      ...
      pts.push(c);
      // AND ITS FACE CENTRES. A plain mesh's bbox centre lies INSIDE the solid, so for a
      // single-mesh hotspot it is occluded by that mesh's own front face and the hotspot
      // reports itself permanently unreachable — which is exactly what the chassis did
      // here. Face centres lie ON the surface, so at least one faces the camera from any
      // angle. Nudged slightly outward so the point is not coplanar with its own face.
      const e = 0.0002;
      pts.push(
        new THREE.Vector3(b.max.x + e, c.y, c.z), new THREE.Vector3(b.min.x - e, c.y, c.z),
        new THREE.Vector3(c.x, b.max.y + e, c.z), new THREE.Vector3(c.x, b.min.y - e, c.z),
        new THREE.Vector3(c.x, c.y, b.max.z + e), new THREE.Vector3(c.x, c.y, b.min.z - e),
      );
    }
  });
  return pts;
...
export function createInteraction({ renderer, camera, scene, sw, state, apply, onAction }) {
  ...
  const HOTSPOTS = [
    {
      id: 'power', label: 'Botón de encendido', node: 'power_button',
      hint: 'Encender o apagar la unidad',
      act: () => { apply.power(state.power !== 'on'); return `power ${state.power}`; },
    },
    {
      id: 'display', label: 'Pantalla táctil', node: 'touch_display',
      hint: 'Tocar para encender o apagar',
      act: () => { apply.power(state.power !== 'on'); return `power ${state.power}`; },
    },
    {
      id: 'rear', label: 'Panel de servicio', node: 'rear_panel',
      hint: 'Volver a la vista frontal',
      act: () => { state.view = state.view === 'rear' ? 'front' : 'rear'; apply.view(); return `vista ${state.view}`; },
    },
    {
      id: 'chassis', label: 'Chasis', node: 'ups_body',
      hint: 'Ver la trasera',
      act: () => { state.view = state.view === 'rear' ? 'front' : 'rear'; apply.view(); return `vista ${state.view}`; },
    },
  ];
