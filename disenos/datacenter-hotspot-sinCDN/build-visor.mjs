...
// Publishes ALONGSIDE the original /p/hotspot/, never over it: the two are meant to be compared
// side by side from the portal — same dashboard, one resolving three from unpkg and this one
// from ./vendor/. Overwriting the original would destroy the control case.
const OUT = join(CINEMEX, 'publish', 'p', 'hotspot-sincdn');
...
/**
 * three stays external to the bundle and is resolved by the page importmap — but the map now
 * points at ./vendor/three/, shipped alongside this build, NOT at unpkg.com. The page therefore
 * makes no request to the public internet, which is the entire reason this variant exists: a
 * corporate proxy that blocks package CDNs leaves the old build with an empty 3D scene.
 */
...
// 0. three.js itself. The importmap in both pages resolves 'three' and 'three/addons/' against
//    ./vendor/three/, so these files must sit next to the HTML or nothing renders. Copied verbatim:
//    three is never bundled and never obfuscated (see the header).
...
// 4. The point of this variant, enforced rather than trusted: NOTHING in the published output may
//    reference a package CDN. If a source edit ever reintroduces the unpkg importmap, the build
//    fails here instead of shipping a dashboard that renders blank behind a corporate proxy.
for (const page of ['index.html', 'rack-detail.html']) {
  // Comments are stripped before the check: the importmap note deliberately NAMES the CDNs it
  // forbids, and a comment never issues a request. Without this, the build fails on its own warning.
  const emitted = readFileSync(join(OUT, page), 'utf8').replace(/<!--[\s\S]*?-->/g, '');
  for (const marker of ['unpkg.com', 'jsdelivr', 'cdnjs', 'esm.sh', 'skypack']) {
    if (emitted.includes(marker)) {
      throw new Error(`${page}: CDN reference "${marker}" in the published output — three must resolve from ./vendor/`);
    }
  }
}
if (!existsSync(join(OUT, 'vendor', 'three', 'three.module.js'))) {
  throw new Error('vendor/three/three.module.js missing from the output — the importmap would resolve to nothing');
}

// 5. Final gate: no readable module tree must ship (there is none in source, but assert anyway).
if (existsSync(join(OUT, 'src'))) throw new Error('publish/p/hotspot-sincdn/src/ exists — the readable module tree must not ship');
