...
- **RUN 9 (REOPEN §8, 2026-08-07, user scope-expansion — NEW AXIS)**: procedural/algorithmic
  ...
  G66 (poisson-disk / WFC / L-systems procedural placement). **G63 (marching cubes / dual contouring) — B50,
  this iteration.** 2 read-only-investigable gaps remain (G64, G66). NOT at STOP.
...
- **Covered blocks**: 50 (B1-B50)
- **Coverage metric**: 46 / 48 closed — **RUN 9 IN PROGRESS (read-only-investigable=2: G64, G66). B45+B46+B47+B48+B49+B50 written.** (Runs 7-8 history below; G41 queued as requires-execution §19 build phase.)
...
- **Open gaps — read-only investigable**: 2 (G64, G66) — **RUN 9 IN PROGRESS** (reopen §8, new axis; B45+B46+B47+B48+B49+B50). NOT at STOP.
...
- Budget cap (safety net): run 1 = 12 blocks (fired). Run 2 (reopen): +7 (B13-B19). Run 7 (reopen): +1 (B40, fired). **Run 9 (reopen): +8 gaps seeded (G59-G66), 6 closed (B45, B46, B47, B48, B49, B50).**
